...
public interface BookMarkRepository extends JpaRepository<BookMark, Long> {
    ...
    // 사용자의 이메일을 기준으로 목록을 반환하는 메소드
    @Query("SELECT bm FROM BookMark bm WHERE bm.user_email = :userEmail")
    List<BookMark> getBookMarksByuserEmail(@Param("userEmail") String userEmail);
