        ...
        <BsBookmarkFill
          ...
          onClick={() => {
            const response = window.confirm(
              '마이페이지에 저장된 스크랩 및 등록한 일정까지 삭제됩니다. 정말 삭제하시겠습니까?',
            );
      ...
      {item.fixed ? (
        <div className={styles.wrapper}>
          ...
          <div className={styles.rightContainer}>
            <div className={`${styles.announcementCategoryDefault} ${styles.announcementCategory}`}>{categoryName}</div>
            <div className={`${styles.announcementDatePostedDefault} ${styles.announcementDatePosted}`}>
              {item.date_posted}
            </div>
            <div className={`${styles.announcementAuthorDefault} ${styles.announcementAuthor}`}>{item.author}</div>
          </div>
        </div>
      ) : (
        <div className={styles.wrapper}>
          <div className={styles.leftContainer}>
            {bookmarkComponent}
            <div className={styles.announcementIdDefault}>{itemNumber}</div>
            <a href={item.url} target="_blank" rel="noopener noreferrer">
              <div className={`${styles.announcementTitleDefault}`}>{item.title}</div>
            </a>
          </div>

          <div className={styles.rightContainer}>
            <div className={`${styles.announcementCategoryDefault}`}>{categoryName}</div>
            <div className={`${styles.announcementDatePostedDefault}`}>{item.date_posted}</div>
            <div className={`${styles.announcementAuthorDefault}`}>{item.author}</div>
          </div>
        </div>
      )}
    </div>
